...
# Updated clothing collections for Limelight
...
def scrape_collection(collection_handle, max_products=None):
    ...
    while True:
        url = f"https://limelight.pk/collections/{collection_handle}/products.json?page={page}&limit=250"
    ...
    for product in products:
        try:
            ...
            processed_product = {
                ...
                'url': f"https://limelight.pk/products/{product.get('handle')}",
                'price_min': min_price,
                'price_max': max_price,
                'available': available,
...
def save_data(processed_data, raw_products):
    ...
    # Save processed JSON
    json_file = f"data/limelight_clothing_{timestamp}.json"
    ...
    summary_file = f"data/limelight_clothing_{timestamp}_summary.txt"
    with open(summary_file, 'w', encoding='utf-8') as f:
        f.write("=" * 60 + "\n")
        f.write("LIMELIGHT CLOTHING COLLECTION - SCRAPING SUMMARY\n")
        f.write("=" * 60 + "\n\n")
...
def main():
    print("\nStarting Limelight clothing scrape (accessories excluded)...\n")
    print("=" * 60)
