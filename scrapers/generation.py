...
# Verified clothing collections for Generation (from site inspection)
CLOTHING_COLLECTIONS = [
    'new-ins',                # New Ins / Summer arrivals (2-piece suits)
    'annual-winter-sale',     # Annual Winter Sale
    'annual-summer-sale',     # Annual Summer Sale
    'formals',                # Formal dresses / 2-piece suits
    'sale',                   # General Sale
    'khaddar-collection',     # Khaddar Collection (unstitched/winter)
    'hand-crafted-2-piece',   # Hand-Crafted 2-Piece (ready-to-wear)
    'luxury-pret',            # Luxury Pret (common; skips if 404)
]
...
def scrape_collection(collection_handle, max_products=None):
    ...
    while True:
        url = f"https://www.generation.com.pk/collections/{collection_handle}/products.json?page={page}&limit=250"
        print(f"   Page {page}... (URL: {url})", end=" ")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                filtered = len(products) - len(clothing_products)
                if filtered > 0:
                    print(f"Got {len(clothing_products)} clothing items (filtered {filtered} accessories)")
                else:
                    print(f"Got {len(clothing_products)} clothing items")
    ...
    for product in products:
        try:
            ...
            processed_product = {
                ...
                'url': f"https://www.generation.com.pk/products/{product.get('handle')}",
                'price_min': min_price,
                'price_max': max_price,
                'available': available,
...
def save_data(processed_data, raw_products):
    ...
    # Save processed JSON
    json_file = f"data/generation_clothing_{timestamp}.json"
    ...
    summary_file = f"data/generation_clothing_{timestamp}_summary.txt"
    with open(summary_file, 'w', encoding='utf-8') as f:
        f.write("=" * 60 + "\n")
        f.write("GENERATION CLOTHING COLLECTION - SCRAPING SUMMARY\n")
        f.write("=" * 60 + "\n\n")
...
def main():
    print("\nStarting Generation clothing scrape (accessories excluded)...\n")
    print("=" * 60)
